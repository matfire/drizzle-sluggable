...
export type SluggableLike = {
  ...
  update(db: DBLike, existing: SelectRow, patch: Partial<InsertRow>): Promise<Partial<InsertRow>>;
  updateFrom(
    db: DBLike,
    existing:
      | SelectRow
      | Promise<SelectRow | null | undefined>
      | (() => SelectRow | null | undefined | Promise<SelectRow | null | undefined>),
    patch: Partial<InsertRow>,
  ): Promise<Partial<InsertRow>>;
  from(
    source: keyof InsertRow | (keyof InsertRow)[] | ((row: ScopeInput) => string),
  ): SluggableLike;
  to(field: keyof InsertRow): SluggableLike;
  withinScope(
    scope: (keyof InsertRow)[] | ((row: ScopeInput) => Record<string, unknown> | undefined),
  ): SluggableLike;
  usingSlugify(slugify: (input: string, separator: string) => string): SluggableLike;
...
export function defineDialectSuite(name: string, createHarness: () => Promise<DialectHarness>) {
  describe(name, () => {
    ...
    it("supports multiple source fields", async () => {
      const sluggable = harness.makeSluggable().from(["title", "categoryId"]).to("slug");
    ...
    it("can load the existing row inside the update helper", async () => {
      ...
      const patch = await sluggable.updateFrom(harness.db, () => harness.getRow(id), {
    ...
    it("supports reserved slugs and max length constraints", async () => {
      ...
      const limitedSluggable = harness.makeSluggable().slugsShouldBeNoLongerThan(12);
    ...
    it("supports custom slugify functions", async () => {
      const sluggable = harness.makeSluggable().usingSlugify((input, separator) => {
        const normalized = input.trim().toLowerCase().replace(/\s+/g, separator);
