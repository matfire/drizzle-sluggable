...
export type CreateSluggableConfig<InsertModel, SelectModel> = {
  ...
  uniqueResolver?: SluggableOptions<InsertModel, SelectModel>["customUniqueResolver"];
...
export type HasDefaultSlugField<InsertModel> = "slug" extends keyof InsertModel ? true : false;

export type HasDefaultIdField<SelectModel> = "id" extends keyof SelectModel ? true : false;

export type ResolveFromState<InsertModel, SelectModel, Config> = Config extends {
  from: SluggableSource<InsertModel, SelectModel>;
}
  ? true
...
type ExistingRowLoader<SelectModel> =
  ...
  | (() => SelectModel | null | undefined | Promise<SelectModel | null | undefined>);
...
> = {
  ...
  withinScope(
    scope: SluggableScope<InsertModel, SelectModel>,
  ): SluggableBuilderApi<InsertModel, SelectModel, Table, HasFrom, HasTo, HasId>;
  usingSeparator(
    separator: string,
  ): SluggableBuilderApi<InsertModel, SelectModel, Table, HasFrom, HasTo, HasId>;
...
> {
  ...
  from(source: SluggableSource<InsertModel, SelectModel>) {
    ...
    return this as unknown as SluggableBuilder<InsertModel, SelectModel, Table, true, HasTo, HasId>;
  }
  ...
  private resolveInsertOptions(data: InsertModel): SluggableOptions<InsertModel, SelectModel> {
    ...
    return {
      ...
      scope: resolveScope<InsertModel, SelectModel>(
        this.config.scope as SluggableScope<InsertModel, SelectModel> | undefined,
...
export function createSluggableCore<
  ...
  SelectModel extends Record<string, unknown>,
  Table extends object,
  Config extends CreateSluggableConfig<InsertModel, SelectModel> | undefined = undefined,
...
async function resolveExistingRow<SelectModel>(
  ...
  const value =
    typeof existing === "function"
      ? await (
          existing as () => SelectModel | null | undefined | Promise<SelectModel | null | undefined>
        )()
      : await existing;
...
>({
  ...
  const rawBase = hasProvidedSlug
    ...
    : buildBaseString<InsertModel>(data, source as SluggableSource<InsertModel>);
  ...
  const finalSlug = ensureNotReserved(uniqueSlug, reserved, separator, maxLength);
...
>({
  ...
  const beforeStr =
    typeof source === "function"
      ...
      : buildBaseString<InsertModel>(existing as any, source as SluggableSource<InsertModel>);
  ...
  const shouldRegenerate =
    onUpdate === "always" || (onUpdate === "updateIfSourceChanged" && sourceChanged);

  const incomingSlugProvided =
    patch[slugField as string] != null && String(patch[slugField as string]).trim() !== "";

  if (!shouldRegenerate && !incomingSlugProvided) {
    return patch;
  }

  const rawBase = incomingSlugProvided ? String(patch[slugField as string] ?? "") : afterStr;

  let base = slugify(rawBase, separator);
  if (!base) base = slugify(`${(existing as any)[idField as string] ?? Date.now()}`, separator);
  if (maxLength) base = base.slice(0, maxLength);
...
function ensureNotReserved(
  ...
  let counter = 2;
  while (true) {
    const nextCandidate = truncate(candidate, `${separator}${counter}`, maxLength);
